...
module.exports = function createSyncUserTeamHandler(topic = '', pubsub, db) {
  ...
  .onPublish(async function syncUserTeamHandler(message, context) {
    ...
    try {
      ...
      log.error(`${LOG_PREFIX} PubSub message was not JSON: ${e} | message ${message.json}`);
      throw Error(`${LOG_PREFIX} PubSub message was not JSON: ${e} | message ${message.json}`);
    }
    ...
    if (currentUser.teams) {
      ...
      const usersUpdatedTeams = usersCurrentTeams.reduce((updatedTeams, teamId) => {
        updatedTeams[teamId] = propertyAndTeam[teamId] ? propertyAndTeam[teamId] : null;
        return updatedTeams;
      }, {});
      ...
      try {
        ...
        const syncErr = Error(`${LOG_PREFIX} error syncing user ${userId} teams: ${err}`);
        log.error(`${syncErr}`);
        throw syncErr;
      }
    }
