...
async function setupTestStructure() {
  ...
  console.log(
    '🏗️ Setting up ClickUp test structure for Sparkle integration...'
  );

  try {
    // Step 1: Check if "Test Property" space exists
    console.log('\n📍 Step 1: Checking for "Test Property" space...');
    const spacesResponse = await clickupService.fetchSpaces(
      apiToken,
      workspaceId
    );

    let testPropertySpace = spacesResponse.spaces.find(
      space => space.name === 'Test Property'
    );

    if (testPropertySpace) {
      console.log(
        `✅ Found existing "Test Property" space (ID: ${testPropertySpace.id})`
      );
    } else {
      console.log('📝 Creating "Test Property" space...');
      testPropertySpace = await clickupService.createSpace(
        apiToken,
        workspaceId,
        {
          name: 'Test Property',
      ...
      console.log(
        `✅ Created "Test Property" space (ID: ${testPropertySpace.id})`
      );
    }

    // Step 2: Check if "Sparkle" folder exists
    console.log('\n📁 Step 2: Checking for "Sparkle" folder...');
    const foldersResponse = await clickupService.fetchFolders(
      apiToken,
      testPropertySpace.id
    );

    let sparkleFolder = foldersResponse.folders.find(
      folder => folder.name === 'Sparkle'
    );

    if (sparkleFolder) {
      console.log(
        `✅ Found existing "Sparkle" folder (ID: ${sparkleFolder.id})`
      );
    } else {
      console.log('📝 Creating "Sparkle" folder...');
      sparkleFolder = await clickupService.createFolder(
        apiToken,
        testPropertySpace.id,
        {
          name: 'Sparkle',
    ...
    const listsResponse = await clickupService.fetchFolderLists(
      apiToken,
      sparkleFolder.id
    );

    let deficientItemsList = listsResponse.lists.find(
      list => list.name === 'Deficient Items'
    );

    if (deficientItemsList) {
      console.log(
        `✅ Found existing "Deficient Items" list (ID: ${deficientItemsList.id})`
      );
    } else {
      console.log('📝 Creating "Deficient Items" list...');
      deficientItemsList = await clickupService.createList(
        apiToken,
        sparkleFolder.id,
        {
          name: 'Deficient Items',
          content:
            'List for testing deficient item creation and management from Sparkle',
        }
      );
      console.log(
        `✅ Created "Deficient Items" list (ID: ${deficientItemsList.id})`
      );
    }
    ...
    let jobsList = listsResponse.lists.find(list => list.name === 'Jobs');
    ...
    const deficientItemsDetails = await clickupService.fetchList(
      apiToken,
      deficientItemsList.id
    );
    ...
    deficientItemsDetails.statuses.forEach((status, index) => {
      console.log(
        `   ${index + 1}. "${status.status}" (${status.type}, color: ${
          status.color
        })`
      );
    });
    ...
    console.log(
      `CLICKUP_TEST_DEFICIENT_ITEMS_LIST_ID=${deficientItemsList.id}`
    );
    ...
    if (error.message.includes('403') || error.message.includes('Forbidden')) {
      console.log(
        '💡 This might be a permissions issue. Check that your API token has admin permissions to create spaces, folders, and lists.'
      );
    }
    process.exit(1);
  }
