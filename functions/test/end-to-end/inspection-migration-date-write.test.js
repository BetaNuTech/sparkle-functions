...
describe('Inspections Migration Date Write', () => {
  ...
  it('should migrate all an inspections\' outdated proxy records', () => co(function *() {
    ...
    yield db.ref(`/inspections/${inspectionId}`).set(assign({}, inspectionData, { migrationDate: now - 1000 })); // Add inspection with old migration
    const beforeSnap = yield db.ref(`/inspections/${inspectionId}/migrationDate`).once('value');
    yield db.ref(`/inspections/${inspectionId}/migrationDate`).set(now);
    const afterSnap = yield db.ref(`/inspections/${inspectionId}/migrationDate`).once('value');
  ...
  it('should update property with any meta data from its\' completed inspections', () => co(function *() {
    ...
    yield db.ref(`/properties/${propertyId}`).set({ name: `name${propertyId}` }); // required
    const beforeSnap = yield db.ref(`/inspections/${inspection1Id}/updatedLastDate`).once('value');
    yield db.ref(`/inspections/${inspection1Id}/updatedLastDate`).set(newest);
    const afterSnap = yield db.ref(`/inspections/${inspection1Id}/updatedLastDate`).once('value');
