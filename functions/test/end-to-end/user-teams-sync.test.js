...
describe('User Teams Sync', () => {
  ...
  it('should add all missing teams and property associations to all users', async () => {
    ...
    await db.ref(`/properties/${property3Id}`).set({ name: 'Mansion', team: team2Id }); // Add property 3 and team 2
    await db.ref(`/properties/${property4Id}`).set({ name: 'Apartment', team: team2Id }); // Add property 4 and team 2

    await db.ref(`/users/${userId}`).set({ firstName: 'Fred', teams: {[team1Id]: { [property1Id]: true }, [team2Id]: { [property3Id]: true } } }); // Add user
    ...
    // Assertions
    expect(actual.val()).to.deep.equal(expectedPayload, `synced /users/${userId}/teams by adding missing`);
  });
  ...
  it('should remove all invalid teams and property associates from all users', async () => {
    ...
    await db.ref(`/users/${user1Id}`).set({ firstName: 'Fred', teams: {
      [team1Id]: { [property1Id]: true, [property2Id]: true },
      [team2Id]: { [property3Id]: true, [property4Id]: true }
    }}); // Add user
    ...
    // Assertions
    expect(actual.val()).to.deep.equal(expectedPayload, `synced /users/${user1Id}/teams by removing invalid`);
  });
  ...
  it('should remove all invalid team/properties associations when all properties have no team associations', async () => {
    ...
    // Assertions
    expect(actual).to.equal(expected, `synced removed property from /users/${userId}/teams by removing invalid`);
  });
});
