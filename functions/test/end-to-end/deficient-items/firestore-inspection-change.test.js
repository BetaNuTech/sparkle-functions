...
describe('Deficient Items | Firestore Inspection Change', () => {
  ...
  it('should not create a new deficient items when matching deficiency already exists', async () => {
    const propertyId = uuid();
    const inspectionId = uuid();
    const itemId = uuid();
    ...
    const expected = 1;
    const inspOnePath = `/inspections/${inspectionId}`;
    const deficiencyData = {
      ...mocking.createDeficientItem(
        inspectionId,
        itemId,
        inspectionData.template.items[itemId]
      ),
      property: propertyId,
    };
