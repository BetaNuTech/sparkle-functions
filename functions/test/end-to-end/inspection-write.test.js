...
describe('Inspection Write', () => {
  ...
  it('should update property meta data with latest state from its\' deficient items', () => co(function *() {
    const insp1Id = uuid();
    const insp2Id = uuid();
    const propertyId = uuid();
    const newest = (Date.now() / 1000);
    ...
    const inspectionTwo = mocking.createInspection({
      ...
      score: 25,
      // Create template w/ 1 deficient item
      template: { items: { [defItemId]: mocking.createCompletedMainInputItem('twoactions_checkmarkx', true) } }
    });
    const expected = {
    ...
    yield db.ref(`/inspections/${insp1Id}`).set(inspectionOne); // Add inspection #1
    yield db.ref(`/inspections/${insp2Id}`).set(inspectionTwo); // Add inspection #2
    yield db.ref(`/propertyDeficientItems/${propertyId}/${defItemId}`).set(mocking.createDeficientItem({ state: 'pending' })); // remove required action state
    const snap = yield db.ref(`/inspections/${insp1Id}`).once('value'); // Create snapshot
