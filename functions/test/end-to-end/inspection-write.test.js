...
describe('Inspection Write', () => {
  ...
  it('should set an invalid score value to zero', () =>
    co(function*() {
      ...
      const results = yield Promise.all(
        paths.map(p => db.ref(p).once('value'))
      );
  ...
  it("should update inspections' proxy records with new data", () =>
    co(function*() {
      const inspectionId = uuid();
      const propertyId = uuid();
  ...
  it('should update completedInspections when inspection becomes completed', () =>
    co(function*() {
      const inspectionId = uuid();
      const propertyId = uuid();
      const now = Date.now() / 1000;
  ...
  it('should ensure an incomplete inspection does not exist in any completed proxy records', () =>
    co(function*() {
      const inspectionId = uuid();
      const propertyId = uuid();
      const now = Date.now() / 1000;
