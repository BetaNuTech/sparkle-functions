  ...
  describe('On Write Handler', function() {
    ...
    it('should not send message when recipient is undiscoverable', function() {
      ...
      return createOnWriteHandler(db, createMessagingStub().value())(
        ...
        expect(loaded).to.equal(true, 'loaded data snapshot successfully')
      );
    });
    ...
    it("should send notification to all a recipients' devices", function() {
      ...
      const messaging = createMessagingStub(
        {},
        {
          sendToDevice: (actualToken, actualPayload) => {
            expect(expectedTokens).to.deep.equal(
              actualToken,
              'sent to all registered tokens'
            );
            expect(expectedPayload.title).to.equal(
              actualPayload.notification.title,
              'sent message title'
            );
            expect(expectedPayload.message).to.equal(
              actualPayload.notification.body,
              'sent message body'
            );
            sentMessage = true;
      ...
      return createOnWriteHandler(db, messaging)(payload, {
        params: { messageId: expectedPayload.recipientId },
      }).then(() =>
        expect(sentMessage).to.equal(
          true,
          'triggered send message successfully'
        )
      );
    });
    ...
    it("should send notification to a recipients' single device", function() {
      ...
      const messaging = createMessagingStub(
        ...
        {
          sendToDevice: (actualToken, actualPayload) => {
            expect([expectedToken]).to.deep.equal(
              actualToken,
              'sent to the registered token'
            );
            expect(expectedPayload.title).to.equal(
              actualPayload.notification.title,
              'sent message title'
